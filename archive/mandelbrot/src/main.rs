...
/// both strigs that can be parsed by 'T;from_str'. 'separator' must be an
/// ASCII character
///
/// If 's' has the proper form, return 'Some<(x, y)>' If it doesn't prase
...
/// image
/// the 'upper_left' and 'lower_right' parameter store points on the complex
...
/// arguments specify points on the complex plane corresponding to the uper-left
...
fn main() {
    ...
    // Bounding for command line args to the function
    if args.len() != 5 {
        eprintln!("Usage: {} FILE PIXELS UPPERLEFT LOWERRIGHT", args[0]);
        eprintln!("Example: {} mandel.png 1000x750 -1.0,0.35 -1,0.20", args[0]);
        std::process::exit(1);
    }
    ...
    // Concurrent modeling with crossbeam
    let threads = 8;
